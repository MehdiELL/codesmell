...
# CodeSmell - AI code-quality reviewer on GenLayer.
# Submit text; GenLayer validators independently evaluate it with an LLM, agree on a label and score by consensus, and write the result on chain. AI code-quality reviewer.
...
SCORE_TOLERANCE = 25
CATEGORIES = ("clean", "minor-issues", "problematic")
DEFAULT_LABEL = "minor-issues"
...
class CodeSmell(gl.Contract):
    ...
    def evaluate(self, item_id: int) -> None:
        ...
        def leader_fn():
            ...
            prompt = f"""You are a senior code reviewer. Judge the quality of the CODE snippet (clarity, correctness signals, obvious smells).
...
Classify the CONTENT as exactly one of: clean, minor-issues, problematic.
Also give an integer smell score from 0 to 100, and a one-sentence reason.

Respond with ONLY JSON: {{"label": "<one of: clean, minor-issues, problematic>", "score": <integer 0-100>, "reasoning": "<one sentence>"}}"""
